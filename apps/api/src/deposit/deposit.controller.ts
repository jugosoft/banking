...
export class DepositController {
    ...
    public constructor(@Inject('DATA_SOURCE') private dataSource: DataSource) {
    ...
    public async getDeposit(
        @Param('id') id: string
    ): Promise<IGetDepositResponse | IResponseErrors> {
    ...
    public async saveDeposit(
        @Body() deposit: IDeposit
    ): Promise<boolean | IResponseErrors> {
        try {
            if (deposit.id) {
